...
static inline int szcyy_bb_w(size_t count, uint8_t *target, struct szc_dgs_s *d) {
  struct szc_dgsw_s *dd = (struct szc_dgsw_s *)d;
  ...
  _szcpy(cdef_SZ_b, dd->val + start, target, count, dd->bitlen - (start << 3));
  dd->bitlen += count;
  return 0;
}
...
int szcy_w(uint8_t typ, size_t count, szcv_t target, struct szc_dgs_s *d) {
  ...
  switch (typ) {
    ...
    case cdef_SZ_b:
      return szcyy_bb_w(count, (uint8_t *)&target, d);
    default:
      return 1;
  }
}
...
int szcyy_w(uint8_t typ, size_t count, uint8_t *target, struct szc_dgs_s *d) {
  ...
  switch (typ) {
    ...
    case cdef_SZ_b:
      return szcyy_bb_w(count, target, d);
    default:
      return 1;
  }
  return 0;
